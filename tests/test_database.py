...
class DatabaseTests(unittest.TestCase):
    def setUp(self):
        ...
        self.alex = self.db.add_member("Alex Instructor", "AI")
    ...
    def test_initialization_and_member_crud(self):
        ...
        self.db.update_member(self.alex, "Alex Updated", "AU")
        self.assertEqual(dict(self.db.list_members()[0])["full_name"], "Alex Updated")
        unused = self.db.add_member("Unused", "U")
        self.db.delete_member(unused)
        self.assertEqual(len(self.db.list_members()), 2)

    def test_unique_initials_and_validation(self):
        with self.assertRaises(sqlite3.IntegrityError):
            self.db.add_member("Duplicate", "ai")
